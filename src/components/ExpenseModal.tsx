...
export default function ExpenseModal({ sourceType, sourceId, emoji, name, onClose }: ExpenseModalProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!numAmount || numAmount <= 0 || !category.trim()) return

    const data = { amount: numAmount, category: category.trim(), note: note.trim(), date }

    if (sourceType === 'pay') {
      await actions.addPayExpense({ ...data, payPeriodId: sourceId })
    } else {
      await actions.addCardExpense({ ...data, cardId: sourceId })
    }
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-2xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-800
                  text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:focus:ring-indigo-500 focus:border-transparent transition-all"
              />
            </div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                type="submit"
                disabled={!amount || Number(amount) <= 0 || !category.trim()}
                className="flex-1 py-3 rounded-2xl font-medium text-white bg-gradient-to-r from-orange-500 to-red-600
                  hover:from-orange-600 hover:to-red-700 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
              >
                💸 Add Expense
              </button>
            </div>
